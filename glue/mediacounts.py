...
t = datetime.strptime(args["ISO_8601_STRING"], "%Y-%m-%dT%H:%M:%S.%f%z") - timedelta(days=2)
...
response = s3_client.upload_file(MCTS_LOCAL_PATH, SOURCE_BUCKET, f"mediacounts/bz/{os.path.basename(MCTS_LOCAL_PATH)}")
job.commit()
...
media_counts = spark.read.option("header", False)\
    .option("delimiter", "\t")\
...
media_counts_slim = media_counts\
    .withColumn("id", lit(obs_uuid()))\
    .withColumn("date", lit(f"{pv_year}-{pv_month:02}-{pv_day:02}"))\
    .withColumn("_date", lit(f"{pv_year}-{pv_month:02}-{pv_day:02}"))\
