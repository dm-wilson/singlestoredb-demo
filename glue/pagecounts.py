...
t = datetime.strptime(args["ISO_8601_STRING"], "%Y-%m-%dT%H:%M:%S.%f%z") - timedelta(days=1)
...
response = s3_client.upload_file(PCTS_LOCAL_PATH, SOURCE_BUCKET, f"pagecounts/gz/{os.path.basename(PCTS_LOCAL_PATH)}")
job.commit()

# read the file from the worker's tmp storage && write input gz file -> dynamic frame
page_counts = spark.read \
    .option("header", False)\
    .option("delimiter", " ")\
    .schema(PCTS_SCHEMA)\
...
page_counts_slim = page_counts\
    .withColumn("id", lit(obs_uuid()))\
    .withColumn("date", lit(PARTITIONING_KEY))\
    .withColumn("interval_start_unixtime", lit(PERIOD_START_UNIXTIME).astype(LongType()))\
